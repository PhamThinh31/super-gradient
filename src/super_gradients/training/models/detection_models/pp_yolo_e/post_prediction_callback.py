...
class PPYoloEPostPredictionCallback(DetectionPostPredictionCallback):
    ...
    def forward(self, outputs, device: str):
        ...
        #     pred_bboxes= prediction_tuple[0]
        #     pred_scores = prediction_tuple[1]
...
class PPYoloEPostPredictionCallback_ReID(DetectionPostPredictionCallback):
    """Non-Maximum Suppression (NMS) module"""
    ...
    def __init__(self, score_threshold: float, nms_threshold: float, nms_top_k: int, max_predictions: int, multi_label_per_box: bool = True):
        ...
        :param multi_label_per_box: controls whether to decode multiple labels per box.
                                    ...
                                    False - each anchor can produce only one label of the class with the highest score.
        """
        super(PPYoloEPostPredictionCallback_ReID, self).__init__()
        self.score_threshold = score_threshold
        self.nms_threshold = nms_threshold
        self.nms_top_k = nms_top_k
        self.max_predictions = max_predictions
    ...
    def forward(self, outputs, device: str):
        ...
        for pred_bboxes, pred_scores, pred_reid  in zip(*predictions):
        # for prediction_tuple in predictions:
        #     pred_bboxes= prediction_tuple[0]
        #     pred_scores = prediction_tuple[1]
            # pred_bboxes [Anchors, 4],
            ...
            if self.multi_label_per_box:
                ...
                pred_cls_conf = pred_scores[i, j]
                pred_cls_label = j[:]
                pred_reid = pred_reid[i]  # Apply indices to pred_reid
                ...
                pred_cls_label = pred_cls_label[conf_mask]
                pred_bboxes = pred_bboxes[conf_mask, :]
                pred_reid = pred_reid[conf_mask]  # Apply mask to pred_reid
            ...
            if pred_cls_conf.size(0) > self.nms_top_k:
                ...
                pred_cls_label = pred_cls_label[topk_candidates.indices]
                pred_bboxes = pred_bboxes[topk_candidates.indices, :]
                pred_reid = pred_reid[topk_candidates.indices]  # Apply topk indices to pred_reid
            ...
            pred_cls_conf = pred_cls_conf[idx_to_keep].unsqueeze(-1)
            pred_cls_label = pred_cls_label[idx_to_keep].unsqueeze(-1)
            pred_reid = pred_reid[idx_to_keep]  # Apply NMS kept indices to pred_reid
            pred_bboxes = pred_bboxes[idx_to_keep, :]

            #  nx6 (x1, y1, x2, y2, confidence, class) in pixel units
            final_boxes = torch.cat([pred_bboxes, pred_cls_conf, pred_cls_label, pred_reid], dim=1)  # [N,6]
