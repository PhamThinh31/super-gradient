...
@register_detection_module()
class YoloNASDFLHead_ReID(BaseDetectionModule, SupportsReplaceNumClasses):
    def __init__(
        self,
        in_channels: int,
        inter_channels: int,
        ...
        first_reid_conv = [ConvBNReLU(inter_channels, inter_channels, kernel_size=3, stride=1, padding=1, groups=groups, bias=False)] if groups else []
        self.reid_convs = nn.Sequential(*first_reid_conv, ConvBNReLU(inter_channels, inter_channels, kernel_size=3, stride=1, padding=1, bias=False))

        self.cls_pred = nn.Conv2d(inter_channels, self.num_classes, 1, 1, 0)
        self.reg_pred = nn.Conv2d(inter_channels, 4 * (reg_max + 1), 1, 1, 0)
        self.reid_pred = nn.Conv2d(inter_channels, 128, 1, 1, 0)

        self.cls_dropout_rate = nn.Dropout2d(cls_dropout_rate) if cls_dropout_rate > 0 else nn.Identity()
        self.reg_dropout_rate = nn.Dropout2d(reg_dropout_rate) if reg_dropout_rate > 0 else nn.Identity()
        # self.reid_dropout_rate = nn.Dropout2d(reid_dropout_rate) if reid_dropout_rate > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        reid_feat = self.reid_convs(x)
        reid_output = self.reid_pred(reid_feat)

        return reg_output, cls_output, reid_output
...
@register_detection_module()
class NDFLHeads_ReID(BaseDetectionModule, SupportsReplaceNumClasses):
    def __init__(
        self,
        num_classes: int,
        in_channels: Tuple[int, int, int],
        ...
        :param width_mult: A scaling factor applied to in_channels.
        """
        super(NDFLHeads_ReID, self).__init__(in_channels)
        in_channels = [max(round(c * width_mult), 1) for c in in_channels]
    ...
    def forward_train(self, feats: Tuple[Tensor, ...]):
        ...
        reid_feat_list, cls_score_list, reg_distri_list = [], [], []
        for i, feat in enumerate(feats):
            reg_distri, cls_logit, reid_logit = getattr(self, f"head{i + 1}")(feat)
            # cls and reg
            # Note we don't apply sigmoid on class predictions to ensure good numerical stability at loss computation
            cls_score_list.append(torch.permute(cls_logit.flatten(2), [0, 2, 1]))
            reid_feat_list.append(torch.permute(reid_logit.flatten(2), [0, 2, 1]))
            reg_distri_list.append(torch.permute(reg_distri.flatten(2), [0, 2, 1]))
        reid_feat_list = torch.cat(reid_feat_list, dim=1)
        cls_score_list = torch.cat(cls_score_list, dim=1)
        reg_distri_list = torch.cat(reg_distri_list, dim=1)

        return cls_score_list, reg_distri_list, reid_feat_list, anchors, anchor_points, num_anchors_list, stride_tensor

    def forward_eval(self, feats: Tuple[Tensor, ...]) -> Tuple[Tuple[Tensor, Tensor], Tuple[Tensor, ...]]:

        cls_score_list,reid_feat_list , reg_distri_list, reg_dist_reduced_list = [], [], [], []
        ...
        for i, feat in enumerate(feats):
            ...
            # cls and reg
            reid_feat_list.append(reid_logit.reshape([b, 128, height_mul_width]))
            cls_score_list.append(cls_logit.reshape([b, self.num_classes, height_mul_width]))
            reg_dist_reduced_list.append(reg_dist_reduced)
        ...
        reid_feat_list = torch.cat(reid_feat_list, dim=-1)  # [B, C, Anchors]
        reid_feat_list = torch.permute(reid_feat_list, [0, 2, 1])  # # [B, Anchors, C]
        ...
        decoded_predictions = pred_bboxes, pred_scores, reid_feat_list
        ...
        raw_predictions = cls_score_list, reg_distri_list, reid_feat_list, anchors, anchor_points, num_anchors_list, stride_tensor
        return decoded_predictions, raw_predictions
