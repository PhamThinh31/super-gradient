...
from super_gradients.training.models.detection_models.pp_yolo_e import PPYoloEPostPredictionCallback, PPYoloEPostPredictionCallback_ReID
from super_gradients.training.utils import get_param
from super_gradients.training.utils.utils import HpmStruct
from super_gradients.training.pipelines.pipelines import DetectionPipeline, DetectionPipeline_ReID
...
@register_model(Models.YOLO_NAS_S_REID)
class YoloNAS_S_REID(YoloNAS):
    def __init__(self, arch_params: Union[HpmStruct, DictConfig]):
        default_arch_params = get_arch_params("yolo_nas_s_reid_arch_params")
        merged_arch_params = HpmStruct(**copy.deepcopy(default_arch_params))
        merged_arch_params.override(**arch_params.to_dict())
        super().__init__(
            backbone=merged_arch_params.backbone,
    ...
    @staticmethod
    def get_post_prediction_callback(conf: float, iou: float) -> PPYoloEPostPredictionCallback_ReID:
        return PPYoloEPostPredictionCallback_ReID(score_threshold=conf, nms_threshold=iou, nms_top_k=1000, max_predictions=300)
    ...
    @lru_cache(maxsize=1)
    def _get_pipeline(self, iou: Optional[float] = None, conf: Optional[float] = None, fuse_model: bool = False) -> DetectionPipeline_ReID:
        ...
        pipeline = DetectionPipeline_ReID(
            model=self,
...
@register_model(Models.YOLO_NAS_M_REID)
class YoloNAS_M_REID(YoloNAS):
    def __init__(self, arch_params: Union[HpmStruct, DictConfig]):
        default_arch_params = get_arch_params("yolo_nas_m_reid_arch_params")
        merged_arch_params = HpmStruct(**copy.deepcopy(default_arch_params))
        merged_arch_params.override(**arch_params.to_dict())
        super().__init__(
            backbone=merged_arch_params.backbone,
