...
class COCOTracking(COCO):
    def __init__(self, annotation_file=None):
        # Initialize the parent class
        super(COCOTracking, self).__init__(annotation_file)

        # Load sequences and extended annotations if they exist
        if 'sequences' in self.dataset:
            self.createIndexSequences()

        if 'annotations' in self.dataset:
            for ann in self.dataset['annotations']:
                if 'track_id' not in ann:
                    ann['track_id'] = -1  # Default value for non-tracked objects

    def createIndexSequences(self):
        # Create an index for sequences for faster querying
        self.sequenceToImgs = {}
        for sequence in self.dataset['sequences']:
            for img_id in sequence['image_ids']:
                if img_id not in self.sequenceToImgs:
                    self.sequenceToImgs[img_id] = []
                self.sequenceToImgs[img_id].append(sequence)

    def getSequences(self, img_ids=[]):
        """
        Get sequences for specific image IDs
        Args:
            img_ids (list of int): list of img ids to get the sequences for
        Returns:
            sequences (list of dict): list of sequences corresponding to the provided img_ids
        """
        sequences = []
        for img_id in img_ids:
            if img_id in self.sequenceToImgs:
                sequences.extend(self.sequenceToImgs[img_id])
        return sequences
...
class COCOFormatDetectionDataset_withTracking(DetectionDataset):
    """Base dataset to load ANY dataset that is with a similar structure to the COCO dataset.
    - Annotation file (.json). It has to respect the exact same format as COCO, for both the json schema and the bbox format (xywh).
    - One folder with all the images.
    ...
    def _init_coco(self) -> COCOTracking:
        annotation_file_path = os.path.join(self.data_dir, self.json_annotation_file)
        if not os.path.exists(annotation_file_path):
            raise ValueError("Could not find annotation file under " + str(annotation_file_path))
        ...
        if not self.verbose:
            with redirect_stdout(open(os.devnull, "w")):
                coco = COCOTracking(annotation_file_path)
        else:
            coco = COCOTracking(annotation_file_path)
    ...
    def _load_annotation(self, sample_id: int) -> dict:
        ...
        target = np.zeros((len(non_crowd_annotations), 6))
        num_seg_values = 98 if self.tight_box_rotation else 0
        target_segmentation = np.ones((len(non_crowd_annotations), num_seg_values))
        target_segmentation.fill(np.nan)
        for ix, annotation in enumerate(non_crowd_annotations):
            ...
            target[ix, 0:4] = annotation["clean_bbox"]
            target[ix, 4] = cls
            target[ix, 5] = annotation['track_id']
        ...
        crowd_target = np.zeros((len(crowd_annotations), 6))
        for ix, annotation in enumerate(crowd_annotations):
            cls = self.class_ids.index(annotation["category_id"])
            crowd_target[ix, 0:4] = annotation["clean_bbox"]
            crowd_target[ix, 4] = cls
            crowd_target[ix, 5] = annotation['track_id']
